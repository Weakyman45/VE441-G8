...
class MainActivity : Activity() {
    ...
    // Live streaming need-analysis shown on the "shopping brief" (CONTEXT) page.
    ...
    private fun analysisProcessCard(): View = card(paleBlue, Color.rgb(171, 210, 229)).apply {
        addView(LinearLayout(this@MainActivity).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER_VERTICAL
            addView(text("AI 分析过程", 15f, ink, Typeface.BOLD), LinearLayout.LayoutParams(0, -2, 1f))
            val status = when {
                briefAnalyzing -> "分析中"
            ...
            conn.inputStream.bufferedReader().use { reader ->
                while (runId == briefAnalysisRun) {
                    ...
                    when (obj.optString("type")) {
                        "delta" -> {
                            val chunk = obj.optString("text")
                            if (chunk.isNotEmpty()) handler.post {
                                if (runId != briefAnalysisRun) return@post
                                briefAnalysisText += chunk
                                if (stage == Stage.CONTEXT && !briefAnalysisPaused) render()
                            }
                        }
                        "done" -> {
                            ...
                            handler.post {
                                ...
                                briefAnalysisPaused = false
                                briefAnalysisDone = true
                                if (briefAnalysisText.isBlank()) {
                                    briefAnalysisText = "(未连接大模型，已使用基础分析)"
                                }
            ...
            handler.post {
                ...
                result
                    .onSuccess { response ->
                        ...
                        briefAnalysisPaused = false
                        briefAnalysisDone = true
                        if (briefAnalysisText.isBlank()) briefAnalysisText = "(已使用基础分析)"
