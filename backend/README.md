...
一个商品 API + Talker-Worker 后端,数据来自 Amazon Reviews 2023(Electronics 类目里筛出的笔记本)。
基础 REST 商品接口只用 Python 标准库(`http.server` + `sqlite3` + `json`)即可运行；如果启用 Realtime 本地 ONNX VAD,
需要安装 `backend/requirements.txt` 里的 `numpy` 和 `onnxruntime`。
