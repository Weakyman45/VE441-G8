...
def plan(session: SessionState) -> dict[str, Any]:
    ...
    base = {
        ...
        "query_hint": session.preference.raw_query or session.preference.use_case or "laptop",
        "planner": "rules",
    }
    ...
    try:
            ...
            [
                {
                    "role": "system",
                    "content": (
                        "You are the Planner agent for VoiceShop++ laptop shopping. "
                        "Return ONLY a JSON object with keys: "
                        "query_hint (short English/Chinese search keywords for catalog), "
                        "reason (one short sentence), "
                        "focus (array of soft constraints like display/battery/os). "
        ...
        if isinstance(data.get("query_hint"), str) and data["query_hint"].strip():
            base["query_hint"] = data["query_hint"].strip()
